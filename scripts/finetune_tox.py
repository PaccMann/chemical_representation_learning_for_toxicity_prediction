...
def main(
    ...
    try:
        ...
        try:
            weight_path = os.path.join(
                model_path, "weights", params.get("weights_name", "best_ROC-AUC_mca.pt")
            )
            model.load(weight_path)
        except Exception:
            try:
                wp = os.listdir(os.path.join(model_path, "weights"))[0]
                logger.info(f"Weights {weight_path} not found. Try restore {wp}")
    ...
    for idx, (name, param) in enumerate(model.named_parameters()):
        logger.info((idx, name, param.shape, f"Gradients: {param.requires_grad}"))
    ...
    logger.info(f"Number of parameters: {num_params} (trainable {num_t_params}).")
    ...
    for epoch in range(params["epochs"]):
        ...
        if test_roc_auc_a > max_roc_auc:
            ...
            with open(os.path.join(ft_model_path, "results", "metrics.json"), "w") as f:
                json.dump(info, f)
