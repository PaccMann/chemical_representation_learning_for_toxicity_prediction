...
parser.add_argument(
    '-c', '--confidence', default=False, action='store_true',
...
def main(
    ...
    preds, labels, attention_scores, smiles, encodings = [], [], [], [], []
    for smiles_batch, labels_batch in tqdm(loader, total=len(loader)):
        ...
        num_tasks = labels_batch.shape[1]
        encodings.append(pred_dict["encodings"])
    ...
    fpr, tpr, threshs = roc_curve(labels, preds)
    roc_auc = auc(fpr, tpr)
    prs = average_precision_score(labels, preds)
    logger.info(f"ROC-AUC = {roc_auc:.5f}")

    youden = threshs[np.argmax(tpr - fpr)]
    # print(youden, np.array(preds > youden))
    bin_preds = preds > youden
    report = classification_report(labels, bin_preds)
    f1 = f1_score(labels, bin_preds)
    acc = accuracy_score(labels, bin_preds)
    bal_acc = balanced_accuracy_score(labels, bin_preds)
    res = {
        "roc-auc": roc_auc,
